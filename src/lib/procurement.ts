...
/**
 * Discipline tints resolve through CSS variables so they follow the active
 * theme instead of being frozen as light-mode hex values.
 */
function discToken(slug: string): { bg: string; color: string } {
  return { bg: `var(--disc-${slug}-bg)`, color: `var(--disc-${slug}-fg)` };
}

export const DISCIPLINE_COLORS: Record<string, { bg: string; color: string }> = {
  Process:    discToken('process'),
  Mechanical: discToken('mechanical'),
  Piping:     discToken('piping'),
  Electrical: discToken('electrical'),
  Instrument: discToken('instrument'),
  Civil:      discToken('civil'),
  Structural: discToken('structural'),
  HSE:        discToken('hse'),
};

export function getDisciplineStyle(disc: string): { bg: string; color: string } {
  return DISCIPLINE_COLORS[disc] ?? discToken('default');
}

/**
 * Tailwind classes per status, used by badges, rails and chips.
 * Every class is written out in full — Tailwind only emits what it can see in
 * the source, so these must never be assembled from fragments at runtime.
 */
export const STATUS_CLASSES: Record<
  ItemStatus,
  { chip: string; rail: string; dot: string; border: string }
> = {
  planning: {
    chip: 'bg-planning-bg text-planning-fg', rail: 'bg-planning',
    dot: 'bg-planning', border: 'border-l-planning',
  },
  ontrack: {
    chip: 'bg-ontrack-bg text-ontrack-fg', rail: 'bg-ontrack',
    dot: 'bg-ontrack', border: 'border-l-ontrack',
  },
  atrisk: {
    chip: 'bg-atrisk-bg text-atrisk-fg', rail: 'bg-atrisk',
    dot: 'bg-atrisk', border: 'border-l-atrisk',
  },
  late: {
    chip: 'bg-late-bg text-late-fg', rail: 'bg-late',
    dot: 'bg-late', border: 'border-l-late',
  },
  onsite: {
    chip: 'bg-onsite-bg text-onsite-fg', rail: 'bg-onsite',
    dot: 'bg-onsite', border: 'border-l-onsite',
  },
};
